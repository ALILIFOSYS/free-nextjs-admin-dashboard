...
export default function InstructorTable({ InstructorData, onPageChange, handleAction }) {
  ...
  const handleYes = async () => {
    const deleteChapter = await axios.delete(`${BaseUrl}/instructors/delete-instructor/${deleteId}`,
      {
        headers: {
  ...
  return (
    <>
      ...
      <div className="max-w my-5 ">
        <button className="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded" onClick={handleCreate}>Create Instructor</button>
      </div>
                      ...
                      <div className="flex items-center">
                        <div className=" overflow-hidden rounded">
                          <Image
                            width={50}
                            height={50}
                            src={`${AWS_STUDENT_BASE_URL}${value.img_url}`}
                            alt="fds"
                          />
                        </div>
                        <div>
